...
bliss
# What are these data? Type ?bliss

# Optimize manually with no information.
# Create the model matrix
X <- sparse.model.matrix(cbind(dead,alive) ~ conc,data = bliss)
binomloglik <- function(beta) {
  # These steps are THE SAME as for the normal case from lecture 1
  # Except now instead of mu = X %*% beta, it's mu (which here is "p")
  # (...actually, let's change it to mu, more clear)
  # mu = inverse link function (X %*% beta)
  # which here is "ilogit"
  # Notice how the below outputs a matrix
  # Need to change it to numeric.
  mu <- as.numeric(ilogit(X %*% beta)) # Inverse link function
  # Then take response distribution (binomial) and plug in mu
  # to compute log likelihood
  # sum(dbinom(...,log = TRUE)) computes the log likelihood.
  # Type ?dbinom for documentation.
  sum(dbinom(x = bliss$dead,size = bliss$dead + bliss$alive,prob = mu,log = TRUE))
}

optfunc <- function(x) -binomloglik(x)
opt <- nlm(f = optfunc,p = rep(0,ncol(X)))
opt$estimate
...
irwls_binomial_glm <- function(y,n,dat) {
  ...
  # Try changing this value and seeing how the answer changes
  mu <- y
...
# Deviance?
myloglik <- -2 * binomloglik(glm_coef)
sat_mod <- glm(cbind(dead,alive) ~ as.factor(conc),data = bliss,family = binomial)
summary(sat_mod)
satmodprobs <- ilogit(model.matrix(sat_mod) %*% coef(sat_mod))
# Compare to unconstrained MLE
bliss$dead/(bliss$dead + bliss$alive)
satloglik <- -2 * sum(dbinom(x = bliss$dead,size = bliss$dead + bliss$alive,prob = satmodprobs,log = TRUE))
myloglik - satloglik
summary(glm_mod)$deviance
