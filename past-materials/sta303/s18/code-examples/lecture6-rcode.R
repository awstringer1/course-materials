...
# E.g. marginal mean of particle = yes:
exp(coef(pois1)['(Intercept)'] + coef(pois1)['particleyes'])
...
# Because, model with interaction is saturated
# Which is the "dependence model"
# So, testing reduced model vs saturated ==> testing independence
...
# Try the saturated model, to see
pois2 <- glm(y ~ particle*quality,data = wafer,family = poisson)
summary(pois2)
# Zero residual deviance
# So comparing the no-interaction model to the saturated is 
# test of independence of particle, quality
...
# Rank == dimension, i.e "full rank" AND SQUARE, i.e. has no null-space.
# So X^t(y - mu) = 0 implies that y = mu.
...
# Or the assignment dataset, from Davison:
library(SMPracticals)
data(smoking)
sum(smoking$alive) + sum(smoking$dead) # Same dataset
